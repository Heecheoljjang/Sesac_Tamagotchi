//
//  UIColor+Extension.swift
//  Tamagotchi
//
//  Created by HeecheolYoon on 2022/07/22.
//

import UIKit

extension UIColor {
    
    static let sesacBackground = UIColor(red: 245/255, green: 252/255, blue: 252/255, alpha: 1)
    
    static let sesacBorder = UIColor(red: 77/255, green: 106/255, blue: 120/255, alpha: 1)
    
    static let cancelColor = UIColor(red: 227/255, green: 236/255, blue: 238/255, alpha: 1)
    
    static let labelBackgroundColor = UIColor(red: 238/255, green: 249/255, blue: 249/255, alpha: 1)
}
